...
| 语法 | 代码 |显示效果 | 备注 |
| ----- | ----- | ------ | ----- |
| \stacrel{上位符号}{基位符号} | \vec{x}\stackrel{\mathrm{def}}{=}{x_1,\dots,x_n} | $\vec{x}\stackrel{\mathrm{def}}{=}{x_1,\dots,x_n}$ |
| \underline | \underline {ABC} | $\underline {ABC}$ |
| \overbrace | \overbrace{1+1+...+1}^{n个} = n | $\overbrace{1+1+...+1}^{n个} = n$ |
| \underbrace | \underbrace{1+1+...+1}_{n个} = n | $\underbrace{1+1+...+1}_{n个} = n$ |
