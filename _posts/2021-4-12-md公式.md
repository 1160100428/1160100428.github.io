...
公式内的字母默认为数学斜体，数字默认为正体，在使用微分符号 $\mathrm d$、二项式系数 $\mathrm C$、等于号上的 $\mathrm {def}$、自然常数 $\mathrm e$、虚数单位 $\mathrm i$ 等情况下，需要把这些字母改为正体。
...
| $\color{#080}{竖直对齐}$| \vdots | \vdots | $\vdots$ |
| $\color{#080}{斜方向对齐}$ | \ddots | \ddots | $\ddots$ |

竖直、斜方向对齐的省略号常见于矩阵中，详见[矩阵](#57-矩阵)
...
| ---- | ---- | ---- | ---- |
| $\color{#080}{开二次方}$ | \sqrt | \sqrt 8 = 2 \sqrt | $\sqrt 8 = 2 \sqrt 2 $ |
| $\color{#080}{开 \ x \ 次方}$ | \sqrt [x] | \sqrt [3] 8 = 2 | $\sqrt [3] 8 = 2$ |
...
| $\color{#A80}{运算}$ | $\color{#A80}{语法}$ | $\color{#A80}{示例代码}$ | $\color{#A80}{显示效果}$ |
| ----- | ----- | ----- | ----- |
| $\color{#080}{极限}$ | \lim | \lim_{x \to \infty} f(x) | $\lim_{x \to \infty} f(x)$ |
| $\color{#080}{规范上下标位置}$ | \lim\limits | \lim\limits_{x \to \infty} f(x) | $\lim\limits_{x \to \infty} f(x)$ |

| $\color{#A80}{运算}$ | $\color{#A80}{语法}$ | $\color{#A80}{显示效果}$ | $\color{#A80}{运算}$ | $\color{#A80}{语法}$ | $\color{#A80}{显示效果}$ |
| ----- | ----- | ----- | ----- | ----- | ----- |
| $\color{#080}{最大}$ | \max | $\max$ | $\color{#080}{最小}$ | \min | $\min$ |
| $\color{#080}{上确界}$ | \sup | $\sup$ | $\color{#080}{下确界}$ | \inf | $\inf$ |
| $\color{#080}{上极限}$ | \limsup | $\limsup$ | $\color{#080}{下极限}$ | \liminf | $\liminf$ |
...
质量 $m = 10.1 \mathrm g$   $\qquad$（此处 $m$ 是默认的斜体而数字和单位 $\rm g$ 都是正体）

上文的公式为 m = 10.1 \mathrm g
